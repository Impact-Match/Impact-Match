...
const VerifySuccess = ({ isNgoVerified, isAdmin }) => {
  console.log(isNgoVerified, isAdmin);
  return (
    <div className="flex justify-center items-center h-screen w-full">
      <div className="flex flex-col items-center w-full h-full bg-darkBlue p-8">
        ...
        <p className="text-sm mb-12 text-white text-center">
          Your email has been successfully verified
        </p>
        {!isNgoVerified && (
          <p className="text-sm mb-12 text-yellow-300 text-center">
            Email verification successful, but your account still needs NGO
            admin approval. Please wait for further notification.
          </p>
        )}
        {isAdmin && (
          <p className="text-sm mb-12 text-yellow-300 text-center">
