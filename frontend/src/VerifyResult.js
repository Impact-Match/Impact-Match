    ...
    const verifyEmail = async () => {
      try {
        const response = await axios.get(
          backend + `/auth/verify-email?token=${token}`
        );
        if (response.status === 200) {
          ...
          console.log("frontend: isNGORequired" + isNgoRequired);
          console.log("frontend: Admin " + isAdmin);
          if (userRole === "ngo" && isNgoRequired) {
            const ngoResponse = await axios.get(
              backend + `/auth/verify-ngo?token=${token}`
            );
